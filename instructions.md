...
1. Open your BTCPay Server service dashboard and navigate to the `Actions` tab.
1. Select the "Lightning Node" Action.
1. Select the installed lightning implementation type you would like to use for BTCPay (ie. LND or CLN).
1. Save this setting.
...
Select the "Altcoin Integrations" action from the `Actions` tab on the service dashboard. Toggle the desired altcoin implementation.

### Monero

<!-- TODO update when monero 040 completed -->
...
First, it must be enabled on your BTCPay Server service details page in StartOS. Run the "Enable Plugins" actions from the Actions Tab, and change `Shopify` to "Enabled".
...
Using this plugin requires a clearnet instance of your BTCPay Server store. This can be done using a [Tor reverse proxy](#setting-up-a-tor-reverse-proxy), or the clearnet configuration in StartOS.
